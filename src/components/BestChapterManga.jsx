    ...
    return (
        <div>
            ...
            {mangaData.map(( manga, index) => (
                <div key={index} className={`flex items-center w-auto h-32  w-96 sm:h-32 border-2 rounded-xl my-5 px-4  bg-[#1d1e20] cursor-pointer hover:scale-[102%] transform transition duration-300 
                ease-in-out shadow_custom_red-20 hid ${index === 0 ? 'float' : ''}`} >
                    ...
                    <div className="flex w-full jusitfy-center items-center mx-2">
                        <div className="h-full w-28 mx-3">
                            <img className=" h-24 object-cover" src={`/src${manga.chapter[1].image}`} alt="best_chapter__manga" />
                        </div>
